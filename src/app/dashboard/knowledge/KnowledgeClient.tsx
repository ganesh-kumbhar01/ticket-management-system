...
export default function KnowledgeClient({ initialArticles, isAdmin }: { initialArticles: Article[], isAdmin: boolean }) {
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-white/50 rounded-2xl overflow-hidden shadow-sm flex flex-col min-h-[400px]">
        ...
        <div className="flex-1 p-6">
          {filteredArticles.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-500 dark:text-slate-400 dark:text-slate-400 space-y-4 py-12">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {filteredArticles.map(article => (
                <div key={article.id} className="p-5 rounded-xl border border-white/60 hover:border-blue-300 hover:shadow-md transition-all bg-white/50 dark:bg-slate-900/50 backdrop-blur-md flex flex-col">
                  <h3 className="font-bold text-slate-900 dark:text-white dark:text-white mb-2 line-clamp-1" title={article.title}>{article.title}</h3>
                  <p className="text-sm text-slate-600 dark:text-slate-300 dark:text-slate-300 mb-4 line-clamp-3 flex-1">
                    {article.content}
                  </p>
                  <div className="text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-400 pt-3 border-t border-white/50">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white/80 backdrop-blur-2xl border border-white/50 w-full max-w-2xl rounded-2xl shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4" noValidate>
              <div>
                <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 dark:text-slate-300 mb-1">Article Title</label>
                <input
                  type="text"
                  {...register('title')}
                  className={`w-full h-11 px-3 bg-white dark:bg-slate-900 dark:bg-slate-900 border ${errors.title ? 'border-red-500 focus:ring-red-500/10' : 'border-slate-300 dark:border-slate-700 dark:border-slate-700 focus:ring-blue-600/10 focus:border-blue-600'} rounded-lg focus:outline-none focus:ring-4 text-slate-900 dark:text-white dark:text-white transition-all`}
